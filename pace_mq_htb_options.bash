...
#	fq (for pacing individial sessions).
...
add() {
	...
	do
		...
		#tc class add dev ${DEV}  parent $parent: classid ${CLASS_ID}  htb rate ${RATE}mbit mtu $MTU
		#tc qdisc add dev ${DEV} parent ${CLASS_ID}  fq maxrate ${RATE}mbit limit ${queue_limit} flow_limit ${queue_limit} 
		tc qdisc add dev ${DEV} parent ${CLASS_ID}  tbf rate ${RATE}mbit burst 20000 limit ${queue_limit_bytes}
		##tc qdisc add dev ${DEV} parent ${CLASS_ID}  tbf rate ${RATE}mbit burst 50000 latency 300ms
		#tc qdisc add dev ${DEV} parent ${CLASS_ID}  fq_codel limit ${queue_limit} flows 1024 quantum 1514 memory_limit 32Mb ecn
		#tc qdisc add dev ${DEV} parent ${CLASS_ID}  sfq perturb 10
		#tc qdisc add dev ${DEV} parent ${CLASS_ID}  pfifo limit  ${queue_limit}
		## tc qdisc add dev ${DEV} parent ${CLASS_ID}  fq maxrate ${RATE}mbit limit ${LIMIT} flow_limit ${FLOW_LIMIT} quantum 1514 initial_quantum 1514 pacing
